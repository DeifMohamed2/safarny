const { Booking, Trip, Company } = require('../models');
const { toDocs } = require('../lib/document');
const { normalizeStatus } = require('../lib/status');
const { getSupportMeta } = require('./tickets');
const { getCompanyStats, listCompanies } = require('./companies');
...
async function loadCore() {
  const [bookings, trips, companies, settings] = await Promise.all([
    toDocs(await Booking.find().lean()),
    toDocs(await Trip.find().lean()),
    listCompanies(),
    getSettings(),
  ]);
  return { bookings, trips, companies, settings };
}

async function getPlatformAnalytics(range = '30d', locale = 'en') {
  const { start, end, prevStart, prevEnd } = getRangeBounds(range);
  const { bookings: allBookings, trips: companyTrips, companies, settings } = await loadCore();
  const currentBookings = allBookings.filter((b) => inRange(b.bookingDate, start, end));
  const previousBookings = allBookings.filter((b) => inRange(b.bookingDate, prevStart, prevEnd));

  const confirmed = currentBookings.filter((b) => normalizeStatus(b.status) === 'confirmed');
  const prevConfirmed = previousBookings.filter((b) => normalizeStatus(b.status) === 'confirmed');
  ...
  const [userStats, companyStats, supportMeta, reviewStats] = await Promise.all([
    getUserStats(),
    getCompanyStats(),
    getSupportMeta(),
    getReviewStats(),
  ...
  const bookingStatus = {
    confirmed: currentBookings.filter((b) => normalizeStatus(b.status) === 'confirmed').length,
    pending: currentBookings.filter((b) => normalizeStatus(b.status) === 'pending').length,
    cancelled: currentBookings.filter((b) => normalizeStatus(b.status) === 'cancelled').length,
...
async function getChartData(metric, range = '30d', locale = 'en') {
  ...
  if (metric === 'revenue-bookings') {
    return {
      ...
      summary: { revenue: analytics.kpis.gmv.value, bookings: analytics.kpis.bookings.value },
    };
  }
  ...
  if (metric === 'commission') {
    return { labels: analytics.commissionSeries.map((item) => item.label), data: analytics.commissionSeries.map((item) => item.profit), label: profitLabel };
  }
  if (metric === 'categories') {
    return { labels: analytics.categoryBreakdown.map((item) => item.name), data: analytics.categoryBreakdown.map((item) => item.count), label: bookingsLabel };
  }
  return analytics;
}

async function exportCsv(type) {
  const { bookings, companies } = await loadCore();
  const rows = [];
  if (type === 'bookings') {
    ...
    buildMonthlySeries(bookings, 12).forEach((item) => {
