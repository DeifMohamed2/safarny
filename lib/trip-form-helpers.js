  ...
  const normalized = items
    .map((item, index) => {
      ...
      return {
        ...
        startDate,
        endDate,
        days: Number(item.days) || days,
        nights: Number(item.nights) || nights,
        totalSpots,
        availableSpots,
        rooms,
      };
...
function normalizeTripPayload(body = {}) {
  ...
  return {
    ...
    priceMode,
    travelDates,
    startDate: primary?.startDate || body.startDate || '',
    endDate: primary?.endDate || body.endDate || '',
    days: Number(body.days) || primary?.days || 1,
    nights: Number(body.nights) || primary?.nights || 0,
    beds,
    price,
    totalSeats,
    availableSeats,
