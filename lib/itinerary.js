...
function locationLabel(id, lang = 'en', fallback = '', list = DEFAULT_DESTINATIONS) {
  const found = findDestination(list, id);
  if (found) return destinationLabel(found, lang, list);
  const legacy = STAY_LOCATIONS.find((item) => item.id === String(id || '').toLowerCase());
  if (legacy) return lang === 'ar' ? legacy.ar : legacy.en;
  return fallback || String(id || '').trim();
}
...
function resolveItinerary(trip = {}) {
  ...
  if (String(trip.type || '').toLowerCase() === 'umrah' || String(trip.type || '').toLowerCase() === 'hajj') return [];
...
function localizeItem(item, lang = 'en', list = DEFAULT_DESTINATIONS) {
  const ar = lang === 'ar';
  ...
  const loc = locationLabel(item.location, lang, '', list);
  return {
    ...item,
    title,
    description,
...
function groupItinerary(items = [], lang = 'en', list = DEFAULT_DESTINATIONS) {
  ...
  const groups = stays.map((stay) => {
    ...
    return {
      ...localizeItem(stay, lang, list),
      activities: nested.map((act) => localizeItem(act, lang, list)),
    };
  });
  activities
    .filter((act) => !nestedIds.has(`${act.dayFrom}|${act.title}|${act.titleAr}`))
    .forEach((act) => {
      groups.push({ ...localizeItem(act, lang, list), activities: [] });
    });
  return groups.sort((a, b) => a.dayFrom - b.dayFrom);
}

function staySummary(items = [], lang = 'en', list = DEFAULT_DESTINATIONS) {
  const stays = parseItinerary(items).filter((item) => item.kind === 'stay').sort((a, b) => a.dayFrom - b.dayFrom);
  if (!stays.length) return '';
  const inWord = lang === 'ar' ? 'في' : 'in';
  return stays
    .map((stay) => {
      const loc = locationLabel(stay.location, lang, stay.title || stay.titleAr, list);
      return `${dayRangeLabel(stay.dayFrom, stay.dayTo, lang)} ${inWord} ${loc}`;
    })
    .join(' · ');
}

function groupedTripItinerary(trip = {}, lang = 'en', list = DEFAULT_DESTINATIONS) {
  return groupItinerary(resolveItinerary(trip), lang, list);
}
